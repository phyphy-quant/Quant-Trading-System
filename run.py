...
async def main():
    ...
    try:
        ...
        # Channels
        cta_signal_channel = cta_config['channels']['signal_channel']  # "cta_signals"
        cta_exec_channel   = cta_config['channels']['execution_report_channel']  # "cta_executionreports"
        ...
        await strategy_executor.add_strategy(
            ...
            config=cta_config  # 會讀到 hedge_mode, one way mode etc.
        )

        # Order Executor => 要聽 cta_signal_channel, publish cta_exec_channel
        cta_order_executor = OrderExecutor(
            api_key=exchange_config['api_key'],
            api_secret=exchange_config['api_secret'],
            redis_url=f"redis://{redis_config['host']}:{redis_config['port']}",
            hedge_mode=hedge_mode,
            # 你可以在 test_order_executor.py 做成: .start(signal_channel=..., execution_channel=...)
        )
        ...
        # 組裝要訂閱的頻道
        # e.g. [MD]PERP_BTC_USDT-kline_5m, [MD]PERP_BTC_USDT-processed-kline_5m
        market_channels = [f"{symbol}-kline_{timeframe}", f"{symbol}-processed-kline_{timeframe}"]
        private_channels = []
        ...
        tasks = [
            asyncio.create_task(data_publisher.start(
                symbol=symbol,
                market_config=market_config,
                private_config=private_config,
                interval=timeframe
            )),
            # Strategy Executor => 連 redis & 訂閱( market_channels, private_channels )
            asyncio.create_task(
            ...
            # CTA OrderExecutor => 只跟 cta_signals (signal) & cta_executionreports (executionreport) 
            asyncio.create_task(
                cta_order_executor.start(
                    signal_channel=cta_signal_channel,
                    execution_channel=cta_exec_channel
                )
            )
        ]
